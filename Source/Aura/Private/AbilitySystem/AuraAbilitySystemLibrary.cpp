...
bool UAuraAbilitySystemLibrary::MakeWidgetControllerParams(const UObject* WorldContextObject, FWidgetControllerParams& OutWCParamas, AAuraHUD*& OutAuraHUD)
{
	...
	{	
		...
		if (OutAuraHUD)
		{
			...
			OutWCParamas.AttributeSet = AS;
			OutWCParamas.AbilitySystemComponent = ASC;
			OutWCParamas.PlayerState = PS;
			OutWCParamas.PlayerController = PC;
			return true;
		}
	}
	return false;
...
void UAuraAbilitySystemLibrary::InitializeDefaultAttributesFromSaveData(const UObject* WorldContextObject, UAbilitySystemComponent* ASC, ULoadScreenSaveGame* SaveGame)
{
	...
	const FGameplayEffectSpecHandle SecondaryAttributesSpecHandle = ASC->MakeOutgoingSpec(CharacterClassInfo->SecondaryAttributes_Infinite, 1.f, SecondaryAttributeContextHandle);
	ASC->ApplyGameplayEffectSpecToSelf(*SecondaryAttributesSpecHandle.Data.Get());

	FGameplayEffectContextHandle VitalAttributeContextHandle = ASC->MakeEffectContext();
	VitalAttributeContextHandle.AddSourceObject(SourceAvatarActor);
	const FGameplayEffectSpecHandle VitalAttributesSpecHandle = ASC->MakeOutgoingSpec(CharacterClassInfo->VitalAttributes, 1.f, VitalAttributeContextHandle);
	ASC->ApplyGameplayEffectSpecToSelf(*VitalAttributesSpecHandle.Data.Get());
}
...
void UAuraAbilitySystemLibrary::GiveStartupAbilities(const UObject* WorldContextObject, UAbilitySystemComponent* ASC, ECharacterClass CharacterClass)
{
	...
	// DA_CharacterAssetInfo에서 각 캐릭터 클래스에 맞는 CharacterClassDefaultInfo를 가져온다.
	const FCharacterClassDefaultInfo& DefaultInfo = CharacterClassInfo->GetClassDefaultInfo(CharacterClass);
	for (TSubclassOf<UGameplayAbility> AbilityClass : DefaultInfo.StartupAbilities)
	{	
		// 레벨을 가져오기 위한 CombatInterface.
		if (ASC->GetAvatarActor()->Implements<UCombatInterface>())
		{	
			//FGameplayAbilitySpec을 생성하고 CombatInterface에서 가져온 레벨로 설정.
...
bool UAuraAbilitySystemLibrary::IsCriticaldHit(const FGameplayEffectContextHandle& EffectContextHandle)
{
	if (const FAuraGameplayEffectContext* AuraEffectContext = static_cast<const FAuraGameplayEffectContext*>(EffectContextHandle.Get()))
	{
	...
	TArray<AActor*>& OutOverlappingActors, const TArray<AActor*>& ActorsToIgnore, float Radius, const FVector& SphereOrigin)
{
	...
	{
		// 지정된 반경(Radius) 내에 있는 동적 객체들과의 충돌을 감지하여 Overlaps 배열에 추가.
		World->OverlapMultiByObjectType(Overlaps, SphereOrigin, FQuat::Identity, FCollisionObjectQueryParams(FCollisionObjectQueryParams::InitType::AllDynamicObjects), FCollisionShape::MakeSphere(Radius), SphereParams);
		for (FOverlapResult& Overlap : Overlaps)
		{	
			// 오버랩 된 액터가 CombatInterface를 구현하고 있는지, 죽지 않았는지 확인.
...
{	
	...
	{	
		...
		double ClosestDistance = TNumericLimits<double>::Max(); // 설정할 수 있는 최대크기.
		AActor* ClosestActor;
		for (AActor* PotentialTarget : ActorsToCheck)
		{
			...
			if (Distance < ClosestDistance)
			{
				...
				ClosestActor = PotentialTarget; // 가장 가까이 있는 액터와 거리 저장.
			}
		}
		ActorsToCheck.Remove(ClosestActor); // 확인해야하는 배열에서 삭제.
		OutClosestTargets.AddUnique(ClosestActor); // 아웃 배열에 추가.
		++NumTargetsFound;
	}
}
...
{	
	...
	EffectContextHandle.AddSourceObject(SourceAvatarActor);
	SetDeathImpulse(EffectContextHandle, DamageEffectParams.DeathImpulse); // 데스 임펄스
	SetKnockbackForce(EffectContextHandle, DamageEffectParams.KnockbackForce); // 넉 백
